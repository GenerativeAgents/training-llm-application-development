...
- 依存パッケージはサプライチェーン攻撃対策として **公開から 2 週間未満のバージョンを解決対象から除外** している(`llm-app/pyproject.toml` の `[tool.uv] exclude-newer = "2 weeks"`、`web/.npmrc` の `min-release-age=14`)。更新するときは `llm-app/` で `uv lock --upgrade` → `uv sync`、`web/` で `rm package-lock.json && npm install` を実行し、解決された版に `pyproject.toml` / `package.json` を `==` / 完全一致でピン留めし直す。`npm audit` に残る指摘が最新版で直っていても、公開 2 週間を過ぎるまでは取り込まれない。
- `better-sqlite3` は 12 系に留めている。13 系は同梱の prebuilt バイナリを使う設計だが、npm がロックファイル経由のインストールで `node-gyp rebuild` を起動してしまい(npm/cli#9837、WiseLibs/better-sqlite3#1516)、ビルドツールのない環境で `npm ci` が失敗する。両 issue が解決してから上げる。
