...
- ① のサービスは、後述のアカウント登録と API-KEY 発行が実施できれば問題ありません。API 利用時は弊社が AWS にて用意するハンズオン環境からの直接アクセスになるため、貴社のネットワークは経由いたしません。
...
① のサービスについては、お持ちでない場合は事前に無料アカウントの登録と API-KEY の発行をお願いします。
